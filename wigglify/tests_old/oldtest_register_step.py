...
def test_register_step(register_algorithm, tmp_path):
    wigglesets = input_wigglesets()

    context = WiggleProcessorContext(wigglesets[0], temp_working_dir=tmp_path)
    register_step = RegisterStep(apply_mask=True, crop_to_least_common_intersection=True, register_algorithm=register_algorithm)
    call_next = MagicMock()

    # when
    register_step.__call__(context, call_next)
