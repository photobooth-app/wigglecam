...
class EncodeProcessor:
    def __init__(self, encode_fun: Callable[[object, str], bytes]):
        ...
        self._event_proc_shutdown: EventType | None = None
        self._encode_process: Process | None = None
    ...
    def start(self):
        ...
        self._event_proc_shutdown = Event()

        self._encode_process = Process(
            target=encode_fun,
            name="encode_process",
...
def encode_fun(shm_buffer_name, _queue_in: Queue, _queue_out: Queue, _fun_frame_to_image, _event_proc_shutdown: EventType):
    # init
